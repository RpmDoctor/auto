...
from core.adaptive import maybe_auto_tune_params
...
from core.trade import append_trade_log, set_futures_leverage, set_futures_margin_type_isolated
...
def _should_exit(state: BotState, mark: float, params: StrategyParams) -> tuple[bool, str]:
    ...
    if entry_side == "LONG":
        ...
        # 고정 손절
        if mark <= entry_price * (1 - sl):
            return True, "고정손절"
        # 고정 익절
        if mark >= entry_price * (1 + tp):
            return True, "고정익절"
        # 트레일링 스탑 (수익권에서만 작동하도록 조건 추가 가능하지만 여기선 단순 구현)
...
def run_bot(client: Client, settings: Settings) -> None:
    ...
    while True:
        # 일일 손실 제한 체크
        if _check_daily_loss(client, state, params):
            time.sleep(300) # 한도 도달 시 5분 대기
            continue

        # 포지션이 이미 있으면, 그 심볼을 우선으로 관리(동시 포지션 방지)
        pos_symbol, pos_amt = _get_any_open_position(client)
        symbol = pos_symbol or _pick_symbol(client, settings)
        params = load_params(settings)
            ...
            if do_exit:
                ...
                if settings.trading_dry_run:
                    ...
                    append_trade_log(
                        ...
                        quantity=qty,
                        price=filled.get("avgPrice") or filled.get("price") or mark,
